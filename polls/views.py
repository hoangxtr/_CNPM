       ...
       return HttpResponse('<h1 style="color: red" align="center> CÓ CL HACK ĐƯỢC KAO </h1>')
        ...
        for store in ven:
            ...
            for order in list_order:
                if order.date_ordered.month == timezone.now().month and order.date_ordered.year == timezone.now().year:
                    quantity = 0
                    sum = 0;
    ...
    if owner is None:
       return HttpResponse('<h1 style="color: red" align="center> CÓ CL HACK ĐƯỢC KAO </h1>')
        ...
        for orderHistory in list_order_history:
            if orderHistory.date_ordered.year == timezone.now().year and orderHistory.date_ordered.month == timezone.now().month and orderHistory.date_ordered.day == timezone.now().day and orderHistory.complete:
